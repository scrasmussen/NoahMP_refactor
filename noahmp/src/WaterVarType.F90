...
!=== define "flux" sub-type of water (water%flux%variable)
  type :: flux_type
    ...
    real(kind=kind_noahmp) :: EvapSoilSfcLiqAcc          ! accumulated soil surface water evaporation per soil timestep [mm/s * dt_soil/dt_main]
    real(kind=kind_noahmp) :: SoilSfcInflowAcc           ! accumulated water input on soil surface per soil timestep [mm/s * dt_soil/dt_main]
    ...
    real(kind=kind_noahmp), allocatable, dimension(:) :: TranspWatLossSoilAcc  ! accumulated transpiration water loss from soil per soil timestep [mm/s * dt_soil/dt_main]
