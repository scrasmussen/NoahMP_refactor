...
! ----------------------------------------------------------------------
    ...
    if ( SurfaceType == 2 ) then  ! lake point
       ...
       if ( (OptGroundResistanceEvap == 1) .or. (OptGroundResistanceEvap == 4) ) then   ! Sakaguchi and Zeng, 2009
          ...
          ResistanceGrdEvap = L_RSURF / D_RSURF
       ...
       elseif ( OptGroundResistanceEvap == 3 ) then  ! Sellers (1992) adjusted to decrease ResistanceGrdEvap for wet soil
          ResistanceGrdEvap = SnowCoverFrac * 1.0 + (1.0 - SnowCoverFrac) * exp(8.25 - 6.0*SoilEvapFac) 
       endif
       if ( OptGroundResistanceEvap == 4 ) then ! SnowCoverFrac weighted; snow ResistanceGrdEvap set in MPTABLE v3.8
          ResistanceGrdEvap = 1.0 / ( SnowCoverFrac * (1.0/ResistanceSnowSfc) + (1.0 - SnowCoverFrac) * (1.0/max(ResistanceGrdEvap,0.001)) )
       endif
       ...
       RelHumidityGrd = SnowCoverFrac + (1.0 - SnowCoverFrac) * exp(SoilMatPotentialSfc * ConstGravityAcc / (ConstGasWaterVapor * TemperatureGrd))
    endif
