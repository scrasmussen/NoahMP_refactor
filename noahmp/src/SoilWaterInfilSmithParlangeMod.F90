...
! ----------------------------------------------------------------------
    ...
    if ( INFLMAX == 1 ) then ! not active for now as the maximum infiltration is estimated based on table values
       ...
       FSUR = SoilWatConductivitySat(ISOIL) + &
              (GAM * (SoilWatConductivitySat(ISOIL) - SoilWatConductivity) / (exp(GAM*1.0e-05/JJ) - 1.0) )
       ...
       if ( JJ == 0.0 ) then ! infiltration at surface == saturated hydraulic conductivity
          ...
          FSUR = SoilWatConductivitySat(ISOIL) + &
                 (GAM * (SoilWatConductivitySat(ISOIL) - SoilWatConductivity) / (exp(GAM*FACC/JJ) - 1.0) )
       endif
