...
! local variable
    ...
    real(kind=kind_noahmp)           :: SnowThickMin(3)   ! minimum thickness [m] of each snow layer
       ...
       if ( SnowIce(J) <= 0.1 ) then
          if ( J /= 0 ) then
             ...
             if ( NumSnowLayerOld < -1 ) then    ! MB/KM: change to NumSnowLayerNeg
                ...
                PondSfcThinSnwComb = PondSfcThinSnwComb +SnowLiqWater(J)   ! NumSnowLayerNeg WILL GET SET TO ZERO BELOW 
                SnowWaterEquiv     = SnowIce(J)                            ! PondSfcThinSnwComb WILL GET ADDED TO PONDING FROM PHASECHANGE PONDING
                SnowDepth          = ThicknessSnowSoilLayer(J)             ! SHOULD BE ZERO HERE BECAUSE IT WAS CALCULATED FOR THIN SNOW
                SnowLiqWater(J)    = 0.0
                SnowIce(J)         = 0.0
