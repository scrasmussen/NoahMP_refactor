...
!-----------------------------------------------------------------------
    ...
    RespirationWood        = WoodRespCoeff * RespTmp(TemperatureCanopy) * WoodMass * WoodPoolIndex
    ...
    DeathLeaf       = LeafMass * 1.0e-6 * (LeafDeathWaterCoeffVeg * DeathCoeffWater + LeafDeathTempCoeffVeg * DeathCoeffTemp)
    DeathStem       = StemMass * 1.0e-6 * (LeafDeathWaterCoeffVeg * DeathCoeffWater + LeafDeathTempCoeffVeg * DeathCoeffTemp)
    ...
    NetPriProdLeafAdd = max(0.0, CarbonFracToLeaf*CarbonAssim - GrowthRespLeaf - RespirationLeafMaint)
    NetPriProdStemAdd = max(0.0, CarbonFracToStem*CarbonAssim - GrowthRespStem - RespirationStem)
   ...
   !NetPriProdStemAdd = CarbonFracToStem*CarbonAssim - GrowthRespStem - RespirationStem       ! MB: test Kjetil 
    ...
    DeathStem      = min(DeathStem, StemMassMaxChg+NetPriProdStemAdd-TurnoverStem)
    ...
    RespirationPlantTot = RespirationRoot + RespirationWood + RespirationLeafMaint + RespirationStem + &
                          GrowthRespLeaf + GrowthRespRoot + GrowthRespWood + GrowthRespStem                           ! g/m2/s C
    RespirationSoilOrg  = 0.9 * RespirationSoil                                                                       ! g/m2/s C MB: add 0.9* v3.7
