...
!energy balance at surface: SAG=(IRB+SHB+EVB+GHB)
    ...
    IF(FIRE <=0.) stop 'error' !call wrf_error_fatal("STOP in Noah-MP: emitted longwave <0")
...
! Monin-Obukhov stability parameter moz for next iteration
    ...
    IF(ZLVL <= ZPD) THEN
       ...
       stop 'error' !call wrf_error_fatal("STOP in Noah-MP glacier")
    ENDIF
...
! --------------------------------------------------------------------------------------------------
   ...
   IF (ERRSW > 0.01) THEN            ! w/m2
     ...
     stop 'error' !call wrf_error_fatal("Radiation budget problem in NOAHMP GLACIER")
   END IF
   ...
   IF(ERRENG > 0.01) THEN
      ...
      !call wrf_message(trim(message))
      stop 'error'!call wrf_error_fatal("Energy budget problem in NOAHMP GLACIER")
   END IF
   ...
   IF(ABS(ERRWAT) > 0.1) THEN
      ...
      WRITE(message,'("    I      J     END_WB     BEG_WB       PRCP       EDIR      RUNSRF     RUNSUB")')
          ! call wrf_message(trim(message))
           ...
           !call wrf_message(trim(message))
           stop 'error'!call wrf_error_fatal("Water budget problem in NOAHMP GLACIER")
        END IF
#endif
